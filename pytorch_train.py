...
def train(SegNet):
    ...
    loss_func = nn.CrossEntropyLoss(weight=torch.from_numpy(np.array(CATE_WEIGHT)).float()).cpu()
    ...
    for epoch in range(EPOCH):
        for step, (b_x, b_y) in enumerate(train_loader):
            b_x = b_x.cpu()
            b_y = b_y.cpu()
            b_y = b_y.view(BATCH_SIZE, 416, 416)
            output,relu_value = SegNet(b_x)
...
parser.add_argument("--epoch", type=int, default=2, help="训练迭代次数")
