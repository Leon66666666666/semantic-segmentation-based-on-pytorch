...
class SegNet(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.deco5 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.BatchNorm = nn.BatchNorm2d(64, momentum=bn_momentum)
        self.relu =  nn.ReLU()
        self.conv = nn.Conv2d(64, output_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.deco5(x)
        x = self.BatchNorm(x)
        relu_value = self.relu(x)
        x = self.conv(relu_value)
